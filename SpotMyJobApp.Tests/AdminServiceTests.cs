...
namespace SpotMyJobApp.Tests
{
	public class AdminServiceTests
	{
		...
		public async Task AddJobOfferAsync_AddsJobOffer()
		{
			...
			Assert.Equal("Tech Corp", jobOffer.CompanyName);
			Assert.Equal("USA", jobOffer.Country);
			Assert.Single(jobOffer.Sections);
		}
		...
		public async Task ChangeStatusOfApplicationAsync_ChangesStatus()
		{
			...
			await adminService.ChangeStatusOfApplicationAsync(userId, jobOfferId, "Accepted");

			var updatedApplication = await context.JobsApplications.FirstOrDefaultAsync();
			Assert.NotNull(updatedApplication);
			Assert.Equal("Accepted", updatedApplication.Status);
		}
